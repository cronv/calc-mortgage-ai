...
export class MortgageService {
  ...
  readonly totalPayout = computed(() => this.schedule().reduce((s, r) => s + r.payment, 0));
  readonly overpayment = computed(() => Math.max(0, this.totalPayout() - this.loan()));
  ...
  private recompute(): void {
    const i = this.input();
    const payload = { loan: this.loan(), months: i.months, rate: this.effectiveRate(), type: i.paymentType };
    if (this.worker) {
